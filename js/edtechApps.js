...
const HUB_REPO = "edtech";

function appFolder(app) {
  return app.folder;
}

function getAppUrl(app, currentAppId = CURRENT_APP_ID) {
...
function getHubUrl(currentAppId = CURRENT_APP_ID) {
  const override = typeof localStorage !== "undefined" && localStorage.getItem("edtech_hub_base");
  if (override) {
    const base = override.replace(/\/$/, "");
...
function listApps(currentAppId = CURRENT_APP_ID) {
  ...
  return { CURRENT_APP_ID, EDTECH_APPS, HUB_REPO, getAppUrl, getHubUrl, listApps };
})();
